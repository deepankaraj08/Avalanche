...
const Gallery = forwardRef((props, ref) => {
  return (
    <section ref={ref} className="py-20 md:py-44 relative overflow-hidden bg-[#020617] text-white" id="gallery">
      ...
      {/* 1. THE STARS - GPU accelerated */}
      <div className="absolute inset-0 z-0 pointer-events-none transform-gpu">
        <SpaceStars starCount={typeof window !== 'undefined' && window.innerWidth < 768 ? 80 : 150} className="opacity-40 md:opacity-60" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 md:gap-10 space-y-6 md:space-y-10 transform-gpu">
          {GALLERY_IMAGES.map((img, i) => (
            <motion.div
              ...
              className="break-inside-avoid relative group cursor-pointer will-change-transform transform-gpu"
            >
              <div className={`relative overflow-hidden rounded-[1.8rem] md:rounded-[2.5rem] border border-white/5 ${img.size} bg-[#020617]/40 backdrop-blur-md md:backdrop-blur-xl transition-all duration-700 ease-out group-hover:border-white/20 shadow-2xl`}>
                
                <img 
                  src={img.url} 
                  alt={img.title}
                  loading="lazy"
                  className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 grayscale-[30%] lg:grayscale-[60%] group-hover:grayscale-0 opacity-90 transition-all will-change-transform transform-gpu"
                />

                {/* Always visible gradient on mobile, hover-focused on desktop */}
                <div className="absolute inset-0 bg-gradient-to-t from-[#020617] via-[#020617]/20 to-transparent opacity-90 lg:opacity-70 lg:group-hover:opacity-40 transition-opacity duration-500" />

                <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end">
                  {/* Category Tag */}
                  <div className="mb-2 lg:mb-3 lg:translate-y-4 lg:opacity-0 lg:group-hover:translate-y-0 lg:group-hover:opacity-100 transition-all duration-500 delay-75">
                    <span className="px-3 py-1 rounded-lg bg-white/10 backdrop-blur-md border border-white/10 text-[8px] md:text-[9px] font-black uppercase tracking-widest text-white/90">
                      {img.category}
                    </span>
                  </div>

                  {/* Title */}
                  <h3 className="text-lg md:text-3xl font-black text-white tracking-tighter lg:translate-y-4 lg:opacity-0 lg:group-hover:translate-y-0 lg:group-hover:opacity-100 transition-all duration-500 delay-150">
                    {img.title}
                  </h3>
                </div>

                {/* Action Icon - Desktop Only */}
                <div className="absolute top-6 right-6 w-10 h-10 md:w-12 md:h-12 rounded-full bg-white text-black hidden lg:flex items-center justify-center opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100 transition-all duration-500">
